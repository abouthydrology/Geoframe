...
/*	public ExperimentAR2013(Flow basin,TCA area){
	...
	//a file
	public ExperimentAR2013(String basinName,String areaName){
		
		this.arExp=new POCN(basinName,areaName);
...
// This becomes interesting when the program is long and the main should become itself very long and complex	
	...
	public static  ExperimentAR2013 initialize(int netno){
	...
	ExperimentAR2013 experiment=new ExperimentAR2013(basinName,tcaName);
	//Set the name for the outputs
	POCN.outputFilesName=outputDirName+File.separator+netno+"_OCN";
	//Read the flow
	try {
	...
	experiment.arExp.basin.halo();
	experiment.arExp.area.halo();
//	ocn.print();
...
*/
  ...
  public void experimentDeploy(){
	  ...
	  //Write OCN on disk
	 this.outputFlowName=POCN.outputFilesName+"_flow.asc";
	 this.outputTCAName=POCN.outputFilesName+"_tca.asc";
	  ...
	  sI.print();
	  OCNInfo ocnInfo=new OCNInfo.Builder(this.arExp.pEnergyExpenditureExponent,this.arExp.pIterations).workingDirectoryName(POCN.outputFilesName).inputFlowFileName("").inputTCAFileName("").outputDirectoryName(outputFlowName).build();		
	  ocnInfo.print();
	...
	public static void main(String[] args) throws EsriAsciiHeaderException, EofException {
		...
		for(int i=65;i<129;i++){
			...
			ExperimentAR2013 exp=initialize(i);
			//ocn.print();
