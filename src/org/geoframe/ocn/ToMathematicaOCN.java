...
public class ToMathematicaOCN {
	...
	public static OCN read(){
		...
		File test=new File(trial.workingPath+File.separator+trial.shortName+"_OCN_flow.asc");
		while(!test.exists()){
		...
		System.out.print("The files to open are:\n"+trial.workingPath+File.separator+trial.shortName+"_OCN_flow.asc\n");
		System.out.println(trial.workingPath+File.separator+trial.shortName+"_OCN_tca.asc\n");
		String basinName=trial.workingPath+File.separator+trial.shortName+"_OCN_flow.asc";
		String tcaName=trial.workingPath+File.separator+trial.shortName+"_OCN_tca.asc";
		OCN ocn=new OCN(basinName,tcaName);
		//Set the name for the outputs
		ToMathematicaOCN.outputFileName=trial.workingPath+File.separator+trial.shortName+"_OCN.mat";
		//Read the flow
		try {
	...
	public static void writeMath(String outputfileName){
		...
		TextIO.putln("/** This is a turtle file created on "+dateFormat.format(date)+" by ToMathematicaOCN");
		TextIO.putln("* ");
		TextIO.putln("*/");
	...
	public static void main(String[] args) {
		System.out.println("This is ToMathematicaOCN main");
		//Get Working Directory and names
		//Read Flow And TCA
	    OCN ocn=read();		
		//Write a Mathematica File
        //ocn.print();
	    ToMathematicaOCN.data=ToMathematicaOCN.toMathematica(ocn);
	    ToMathematicaOCN.writeMath(ToMathematicaOCN.outputFileName);
