...
public class POCN {
	...
	public POCN(Flow basin,TCA area){
	...
	//a file
	public POCN(String basinName,String areaName){
		// Energy Expenditure Exponent, number of iterations and mode must be set outside
		...
		try {
			this.area = TCA.asciiRasterReader(areaName);
		} catch (EsriAsciiHeaderException e) {
			e.printStackTrace();
		} catch (EofException e) {
			e.printStackTrace();
...
// This becomes interesting when the program is long and the main should become itself very long and complex	
	...
	POCN ocn=new POCN(basinName,tcaName);
	//Set the name for the outputs
	POCN.outputFilesName=outputDirName+File.separator+trial.shortName;
	//Read the flow
	try {
  ...
  private void  setPalette(int[] randomInt,long[][] palette, long i){
		   ...
		   //System.out.println("$%^");
			...
			for(int h=0; h<no;h++){
				...
				if(!isALoop(randomPoint,Flow.nextPossibilities[h],palette,l)){
					...
					POCN.newDir[0]=Flow.nextPossibilities[h][0];
					POCN.newDir[1]=Flow.nextPossibilities[h][1];
					POCN.newDir[2]=Flow.nextPossibilities[h][2];
	...
	private boolean isALoop(int[] nextPoint,int[] nextPossibility,long[][] palette,long l){
	 ...
	 private double deltaEE(int[] randomInt,double pEEE,long[][] palette,long itr){
		 ...
		 newPoint=randomInt;
		 pLocalArea=this.area.tca[newPoint[0]][newPoint[1]];
		 //System.out.println("Local Area "+pLocalArea);
		 newPoint=this.basin.goDownstream(newPoint,true);
		 while(palette[newPoint[0]][newPoint[1]] > - itr){
			 pEDelta+=-Math.pow(this.area.tca[newPoint[0]][newPoint[1]],pEEE)+Math.pow(this.area.tca[newPoint[0]][newPoint[1]]-pLocalArea,pEEE);
	 ...
	 private void reSetOCN(int[] randomInt,long[][] palette,long itr){
		...
		//Varies the tca in the in the old direction;
		 ...
		 //System.out.println("Local Area "+pLocalArea);
		  newPoint=this.basin.goDownstream(newPoint,true);
		  while(palette[newPoint[0]][newPoint[1]] > - itr){
			     this.area.tca[newPoint[0]][newPoint[1]]-=pLocalArea;
			     //do not forget to work on the Halo
			     if(newPoint[1]==this.area.tca[0].length-2){
			    	 this.area.tca[newPoint[0]][0]-=pLocalArea;
			     } else if(newPoint[1]==1){
			    	 this.area.tca[newPoint[0]][this.area.tca[0].length-1]-=pLocalArea;
			     }
				 newPoint=this.basin.goDownstream(newPoint,true);
				 //TextIO.getAnyChar();
			 }
		...
		this.basin.flow[randomInt[0]][randomInt[1]]=POCN.newDir[2];
		if(randomInt[1]==1){
			this.basin.flow[randomInt[0]][this.basin.flow[0].length-1]=POCN.newDir[2];
		}else if(randomInt[1]==this.basin.flow[0].length-2){
			this.basin.flow[randomInt[0]][0]=POCN.newDir[2];
		}//else do nothing
		//Varies the tca in the new direction;
		 newPoint[0]=POCN.newDir[0];
		 newPoint[1]=POCN.newDir[1];
		 if(newPoint[1]==0){
	        	newPoint[1]=this.basin.flow[0].length-2;
	        }else if(newPoint[1]==this.basin.flow[0].length-1){
	        	newPoint[1]=1;
		...
		// System.out.println("rrr"+newPoint[0]+"$"+newPoint[1]);
		 while(palette[newPoint[0]][newPoint[1]] > - itr){
			 this.area.tca[newPoint[0]][newPoint[1]]+=pLocalArea;
			 if(newPoint[1]==this.area.tca.length-2){
				 this.area.tca[newPoint[0]][0]+=pLocalArea;
			 }else if(newPoint[1]==1){
				 this.area.tca[newPoint[0]][this.area.tca.length-1]+=pLocalArea;
			 }//else do nothing
			 //System.out.println("####");
			 newPoint=this.basin.goDownstream(newPoint,true);
			 //TextIO.getAnyChar();
  ...
  public void ocnDeploy(){
	  ...
	  //Write OCN on disk
	  String outputFlowName=POCN.outputFilesName+"_flow.asc";
	  String outputTCAName=POCN.outputFilesName+"_tca.asc";
	  ...
	  String logFileName=POCN.outputFilesName+"_log.txt";	
	  TextIO.writeFile(logFileName);
	  sI.print();
	  OCNInfo ocnInfo=new OCNInfo.Builder(this.pEnergyExpenditureExponent,this.pIterations).workingDirectoryName(POCN.outputFilesName).inputFlowFileName("").inputTCAFileName("").outputDirectoryName(outputFlowName).build();		
	  ocnInfo.print();
	...
	public static void main(String[] args) throws EsriAsciiHeaderException, EofException {
		System.out.println("This is OCN main");
        POCN ocn=POCN.initialize();
        //ocn.print();
        ocn.run();
        ocn.ocnDeploy();
