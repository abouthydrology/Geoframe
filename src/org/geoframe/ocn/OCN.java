...
public class OCN {
	...
	public OCN(Flow basin,TCA area){
	...
	//a file
	public OCN(String basinName,String areaName){
		// Energy Expenditure Exponent, number of iterations and mode must be set outside
...
// This becomes interesting when the program is long and the main should become itself very long and complex	
	...
	OCN.outputFilesName=outputDirName+File.separator+trial.shortName;
	//Read the flow
	try {
		   ...
		   if(no==0){
			...
			for(int h=0; h<no;h++){
				if(!isALoop(randomPoint,Flow.nextPossibilities[h],palette,l)){
					...
					OCN.newDir[0]=Flow.nextPossibilities[h][0];
					OCN.newDir[1]=Flow.nextPossibilities[h][1];
					OCN.newDir[2]=Flow.nextPossibilities[h][2];
	...
	private boolean isALoop(int[] nextPoint,int[] nextPossibility,long[][] palette,long l){
	 ...
	 private double deltaEE(int[] randomInt,double pEEE,long[][] palette,long itr){
	  ...
	  private void reSetOCN(int[] randomInt,long[][] palette,long itr){
		...
		this.basin.flow[randomInt[0]][randomInt[1]]=OCN.newDir[2];
		//Varies the tca in the new direction;
		 newPoint[0]=OCN.newDir[0];
		 newPoint[1]=OCN.newDir[1];
		 while(palette[newPoint[0]][newPoint[1]] > - itr){
			 this.area.tca[newPoint[0]][newPoint[1]]+=pLocalArea;
			 newPoint=this.basin.goDownstream(newPoint);
			 //TextIO.getAnyChar();
  ...
  private void ocnDeploy(){
	  ...
	  //Write OCN on disk
	  String outputFlowName=OCN.outputFilesName+"_flow.asc";
	  String outputTCAName=OCN.outputFilesName+"_tca.asc";
	  ...
	  String logFileName=OCN.outputFilesName+"_log.txt";	
	  TextIO.writeFile(logFileName);
	  sI.print();
	  OCNInfo ocnInfo=new OCNInfo.Builder(this.pEnergyExpenditureExponent,this.pIterations).workingDirectoryName(OCN.outputFilesName).inputFlowFileName("").inputTCAFileName("").outputDirectoryName(outputFlowName).build();		
	  ocnInfo.print();
