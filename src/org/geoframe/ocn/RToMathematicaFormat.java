...
public class RToMathematicaFormat {
	
	private static String outputFileName;
	private static double[][] data;
	
	public static ROCN read(){
		...
		File test=new File(trial.workingPath+File.separator+trial.shortName+"_R_OCN_flow.asc");
		while(!test.exists()){
			System.out.println("Re-enter the files prefix");
			trial.shortName=TextIO.getlnWord();
			test=new File(trial.workingPath+File.separator+trial.shortName+"_R_OCN_flow.asc");			
		}
		System.out.print("The files to open are:\n"+trial.workingPath+File.separator+trial.shortName+"_R_OCN_flow.asc\n");
		System.out.println(trial.workingPath+File.separator+trial.shortName+"_R_OCN_tca.asc\n");
		String basinName=trial.workingPath+File.separator+trial.shortName+"_R_OCN_flow.asc";
		String tcaName=trial.workingPath+File.separator+trial.shortName+"_R_OCN_tca.asc";
		ROCN ocn=new ROCN(basinName,tcaName);
		//Set the name for the outputs
		RToMathematicaFormat.outputFileName=trial.workingPath+File.separator+trial.shortName+".mat";
		//Read the flow
		try {
	...
	public static double[][] toMathematica(ROCN ocn){
		...
		for(int i=1;i<ocn.basin.rows-1;i++){
			for(int j=1;j<ocn.basin.cols-1;j++){
				...
				dataFrame[count][4]=ocn.area.field[i][j];	
				count++;
			}
		}
	...
	public static void main(String[] args) {
        ...
        //ocn.print();
	    RToMathematicaFormat.data=RToMathematicaFormat.toMathematica(ocn);
	    RToMathematicaFormat.writeMath(RToMathematicaFormat.outputFileName);
