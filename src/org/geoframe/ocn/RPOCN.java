...
/**
 * differently from POCN uses a tca with doubles (instead than long)
 * 
...
public class RPOCN {
	...
	public RPOCN(Flow basin,Field area){
	...
	//a file
	public RPOCN(String basinName,String areaName){
		// Energy Expenditure Exponent, number of iterations and mode must be set outside
...
// This becomes interesting when the program is long and the main should become itself very long and complex	
	...
	public static RPOCN initialize(){
	...
	//System.out.println(trial.workingPath+File.separator+trial.shortName+"_ftca.asc\n");
	...
	String outputTCAName=outputDirName+File.separator+trial.shortName+"_ftca.asc";
	...
	RPOCN ocn=new RPOCN(basinName,tcaName);
	//Set the name for the outputs
	RPOCN.outputFilesName=outputDirName+File.separator+trial.shortName;
	//Read the flow
	try {
    ...
    public void run(){
    	...
    	for(int i=1;i<this.pIterations+1;i++){
    	  //Select a point at random inside the network
  ...
  private void  setPalette(int[] randomInt,long[][] palette, long i){
          ...
          while(this.basin.flow[newPoint[0]][newPoint[1]]!=10){
        	  newPoint=this.basin.goDownstream(newPoint,true);
		   ...
		   //System.out.println("$%^");
			...
			for(int h=0; h<no;h++){
				...
				if(!isALoop(randomPoint,Flow.nextPossibilities[h],palette,l)){
					...
					RPOCN.newDir[0]=Flow.nextPossibilities[h][0];
					RPOCN.newDir[1]=Flow.nextPossibilities[h][1];
					RPOCN.newDir[2]=Flow.nextPossibilities[h][2];
	...
	private boolean isALoop(int[] nextPoint,int[] nextPossibility,long[][] palette,long l){
        ...
        //This can maybe avoided ?
        if(newPoint[1]==0){
        	newPoint[1]=this.basin.flow[0].length-2;
        }else if(newPoint[1]==this.basin.flow[0].length-1){
        	newPoint[1]=1;
        }
	 ...
	 private void reSetOCN(int[] randomInt,long[][] palette,long itr){
		...
		//Varies the tca in the in the old direction;
		 ...
		 //System.out.println("Local Area "+pLocalArea);
		  ...
		  while(palette[newPoint[0]][newPoint[1]] > - itr){
			     ...
			     //do not forget to work on the Halo
			     if(newPoint[1]==this.area.field[0].length-2){
			    	 this.area.field[newPoint[0]][0]-=pLocalArea;
			     } else if(newPoint[1]==1){
			    	 this.area.field[newPoint[0]][this.area.field[0].length-1]-=pLocalArea;
			     }
				 newPoint=this.basin.goDownstream(newPoint,true);
				 //TextIO.getAnyChar();
			 }
		...
		this.basin.flow[randomInt[0]][randomInt[1]]=RPOCN.newDir[2];
		if(randomInt[1]==1){
			this.basin.flow[randomInt[0]][this.basin.flow[0].length-1]=RPOCN.newDir[2];
		}else if(randomInt[1]==this.basin.flow[0].length-2){
			this.basin.flow[randomInt[0]][0]=RPOCN.newDir[2];
		}//else do nothing
		//Varies the tca in the new direction;
		 newPoint[0]=RPOCN.newDir[0];
		 newPoint[1]=RPOCN.newDir[1];
		 if(newPoint[1]==0){
	        	newPoint[1]=this.basin.flow[0].length-2;
	        }else if(newPoint[1]==this.basin.flow[0].length-1){
	        	newPoint[1]=1;
	        }
		// System.out.println("rrr"+newPoint[0]+"$"+newPoint[1]);
		 while(palette[newPoint[0]][newPoint[1]] > - itr){
			 this.area.field[newPoint[0]][newPoint[1]]+=pLocalArea;
			 if(newPoint[1]==this.area.field.length-2){
				 this.area.field[newPoint[0]][0]+=pLocalArea;
			 }else if(newPoint[1]==1){
				 this.area.field[newPoint[0]][this.area.field.length-1]+=pLocalArea;
			 }//else do nothing
			 //System.out.println("####");
			 newPoint=this.basin.goDownstream(newPoint,true);
			 //TextIO.getAnyChar();
  ...
  public void ocnDeploy(){
	  
	  //Write OCN on disk
	  String outputFlowName=RPOCN.outputFilesName+"_flow.asc";
	  String outputTCAName=RPOCN.outputFilesName+"_ftca.asc";
	  ...
	  String logFileName=RPOCN.outputFilesName+"_log.txt";	
	  TextIO.writeFile(logFileName);
	  sI.print();
	  OCNInfo ocnInfo=new OCNInfo.Builder(this.pEnergyExpenditureExponent,this.pIterations).workingDirectoryName(RPOCN.outputFilesName).inputFlowFileName("").inputTCAFileName("").outputDirectoryName(outputFlowName).build();		
	  ocnInfo.print();
	...
	public static void main(String[] args) throws EsriAsciiHeaderException, EofException {
		System.out.println("This is RPOCN main");
        RPOCN ocn=RPOCN.initialize();
        //ocn.print();
        ocn.run();
        ocn.ocnDeploy();
